...
def FTBS(phi,nt):
    ...
    for t in xrange(0,nt):
        ...
        for j in xrange(1,nx):
            ...
            c = (phi[j]*dt)/dx
            max_c = np.max([c,max_c]) 
            phi_New[j] = phi[j] - c*(phi[j]-phi[j-1])
        print ('maximum c number at ts ', t, ' is ', max_c)
        # boundary conditions
        phi_New[0] = phi[0] - c*(phi[0]-phi[nx-1])
        phi[nx] = phi[0]
        phi = phi_New.copy()
    return phi_New,phi
    
def CTBS(phi,phi_old,nt):
    phi_New = phi.copy()
    for t in xrange(0,nt):
        for j in xrange(1,nx):
            c = (phi[j]*dt)/dx
            phi_New[j] = phi_old[j] - 2.*c*(phi[j]-phi[j-1])
        phi_New[0] = phi_old[0] - 2.*c*(phi[0]-phi[nx-1])
        phi_New[nx] = phi_New[0]
        phi_old = phi.copy()
        phi = phi_New.copy()
    return phi_New
